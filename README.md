...
cd proofloop
pnpm install
pnpm build             # required once — the CLI links workspace packages by their dist/ output
...
Open `http://localhost:5173` → register a repository (**Register repository** — local path or GitHub/GitLab), run a check, review Evidence / Unknowns / Impact, accept or reject claims. Accepts are SHA-bound and written to `.proofloop/reviews.jsonl`. If the API enforces auth, paste the token via **Set token** in the header (sent as Bearer, stored in the browser only). Confirming a claim from the UI requires a reviewer name and note for the audit trail.
...
```bash
pnpm proofloop init --profile adopt      # advisory: report gaps, don't block on missing evidence
pnpm proofloop init --profile standard  # default blocking gate (critical/high)
pnpm proofloop init --profile strict    # tighter blockOn + more dynamic categories
```

- `policies.mode` — `advisory` (report unknowns, allow merge; **failed tests still block**) or `blocking` (default)
...
- Dashboard **Rules** merge into these policies (preview → save → next check); syncs `proofloop.yml` when `localPath` exists. Rules may only tighten (including `advisory` → `blocking`)
- API lock: `PROOFLOOP_API_TOKEN` (Bearer) protects **all** non-public routes; `PROOFLOOP_API_KEYS="token:orgId,…"` issues tenant-scoped keys. Production refuses to boot without auth credentials
- Org-scoped keys may only register `localPath` checkouts inside their own workspace subtree (`{PROOFLOOP_WORKSPACE_ROOT}/{org-slug}/`)
- Webhook signatures are mandatory: unsigned GitHub/GitLab deliveries are rejected, even in development
- Async checks return immediately; cancel with `POST /api/runs/:id/cancel`; failed runs publish a failing Check Run / commit status (never silent)
- Optional Redis queue: `REDIS_URL=redis://localhost:6379` (BullMQ). Without it, jobs run in-memory in the API process (bounded by `PROOFLOOP_QUEUE_CONCURRENCY`, default 2)
